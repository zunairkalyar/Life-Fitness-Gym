...
export default function LifeFitnessAiCoach({ 
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        let errMsg = "An abnormal response from Gemini service.";
        try {
          const errData = await response.json();
          errMsg = errData.error || errMsg;
        } catch (_) {
          errMsg = `Gateway/Server error (${response.status} ${response.statusText})`;
        }
...
Do not include any descriptive chatter before/after, only return valid raw JSON matching this schema so I can parse it cleanly.`;
    ...
    try {
      ...
      if (!response.ok) {
        let errMsg = "Unable to parse JSON from AI engine.";
        try {
          const errData = await response.json();
          errMsg = errData.error || errMsg;
        } catch (_) {
  ...
  const [saveToDashboardLoading, setSaveToDashboardLoading] = useState(false);

  const handleSaveToActiveDashboard = async () => {
    if (!generatedPlan) return;
    setSaveToDashboardLoading(true);
    try {
      const response = await fetch("/api/member/workouts", {
        ...
        body: JSON.stringify({
          ...
          editPermission: "full",
          exercises: generatedPlan.exercises.map((ex, idx) => ({
            id: `ex-${idx}-${Date.now()}`,
            name: ex.name,
            ...
            notes: String(ex.notes || "Generated by Life Fitness AI Coach"),
            coachInstructions: String(generatedPlan.safetyNote || "")
          }))
        })
      });
      ...
      triggerFeedbackNotification("🌟 Saved directly to your Workout Dashboard Plans!");
    } catch (err) {
      console.error(err);
      alert("Failed to synchronize directly to active workout dashboard. Pinned in local AI schedules list.");
    } finally {
      setSaveToDashboardLoading(false);
    }
  };
          ...
          <div className="bg-neutral-900 border border-neutral-850 p-4.5 rounded-3xl space-y-4 animate-fade-in text-xs">
            ...
            <div className="space-y-2.5 leading-normal">
              ...
              <div className="flex flex-col gap-2 pt-2 border-t border-neutral-800">
                <button
                  type="button"
                  disabled={saveToDashboardLoading}
                  onClick={handleSaveToActiveDashboard}
                  className="w-full py-2 bg-gradient-to-r from-yellow-500 to-amber-600 hover:from-yellow-600 hover:to-amber-700 disabled:bg-neutral-800 text-black font-extrabold uppercase text-[9px] tracking-wider rounded-lg transition-all cursor-pointer flex items-center justify-center gap-1"
                >
                  {saveToDashboardLoading ? "Exporting..." : "⚡ Sync & Add to Member Dashboard Plans"}
                </button>
